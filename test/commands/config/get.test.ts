...
describe('config:get', () => {
  describe("Testing calls made to core's ConfigAggregator.getInfo() method", () => {
    ...
    test
      .stdout()
      .stderr()
      .command(['config:get', 'defaultdevhubusername', 'badarg', '--json', '--verbose', '--loglevel=warn'])
  ...
  describe('Testing console output', () => {
    test
      ...
      .it('Table with both successes and failures', (ctx) => {
        ...
        const noWhitespaceOutput = getOutput.replace(/\s+/g, '');
  ...
  describe('Testing JSON output', () => {
    test
      ...
      .it('Unset keys', (ctx) => {
        ...
        expect(jsonOutput.result[0]).to.have.property('key').and.equal('defaultdevhubusername');
        expect(jsonOutput.result[1]).to.have.property('key').and.equal('defaultusername');
      });
    ...
    test
      ...
      .it('Bad argument error JSON', (ctx) => {
        ...
        expect(jsonOutput).to.have.property('name').and.equal('UnknownConfigKey');
        expect(jsonOutput).to.have.property('message').and.equal('Unknown config key: badarg');
        expect(jsonOutput).to.have.property('exitCode').and.equal(1);
        expect(jsonOutput).to.have.property('commandName').and.equal('Get');
      });
    ...
    test
      ...
      .it('Global keys', (ctx) => {
        ...
        expect(jsonOutput).to.have.property('result');
        expect(jsonOutput.result[0]).to.have.property('key').and.equal('defaultdevhubusername');
        expect(jsonOutput.result[0]).to.have.property('value').and.equal('DevHub');
        expect(jsonOutput.result[0]).to.have.property('location').and.equal('Global');
        expect(jsonOutput.result[0]).to.have.property('path').and.contain('?');
        expect(jsonOutput.result[1]).to.have.property('key').and.equal('defaultusername');
        expect(jsonOutput.result[1]).to.have.property('value').and.equal('TestUser');
        expect(jsonOutput.result[1]).to.have.property('location').and.equal('Global');
        expect(jsonOutput.result[1]).to.have.property('path').and.contain('?');
      });
    ...
    test
      ...
      .it('Local keys', (ctx) => {
        ...
        expect(jsonOutput.result[0]).to.have.property('key').and.equal('defaultdevhubusername');
        expect(jsonOutput.result[0]).to.have.property('value').and.equal('DevHub');
        expect(jsonOutput.result[0]).to.have.property('location').and.equal('Local');
        expect(jsonOutput.result[0])
          .to.have.property('path')
          .and.contain(`local${path.sep}.sfdx${path.sep}sfdx-config.json`);
        expect(jsonOutput.result[1]).to.have.property('key').and.equal('defaultusername');
        expect(jsonOutput.result[1]).to.have.property('value').and.equal('TestUser');
        expect(jsonOutput.result[1]).to.have.property('location').and.equal('Local');
